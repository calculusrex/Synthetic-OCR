...
from text import ascii_alphanumeric, ascii_characters, ascii_digits, ascii_symbols, ascii_alphabet, random_font, max_char_size, random_string, fonts

from image import add_background, adjust_bitmap_n_bounding_box_margins, morphology_ops, noisey_background, noiseyRGBBitmap, adjust_textAlphaBitmap_alpha, add_line_noise, randcol, add_arc_noise
...
def string_2_paragraph_sample(string,
    ...
    fnt = random_font()
    max_char_w, max_char_h = (
        max(map(lambda f: f['max_char_size'][0], fonts)),
        max(map(lambda f: f['max_char_size'][1], fonts))
    )
    font = fnt['font']
...
if __name__ == '__main__':
    print("## PARAGRAPH SYNTHESIS")

    string = random_string(ascii_alphanumeric, np.random.randint(4, 16))

    image, bounding_boxes, angle = string_2_word_sample(
